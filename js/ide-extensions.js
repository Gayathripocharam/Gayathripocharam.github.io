// ==============================================================================
// Gayathri's IDE Extensions & Analytics
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initLiveActivityFeed() {
    ...
    try {
      try {
        const res = await fetch('https://api.github.com/users/Gayathripocharam/events?per_page=15');
        if (!res.ok) throw new Error(`API error ${res.status}`);
        events = await res.json();
      } catch (err) {
        console.warn("Falling back to local events data...", err);
        try {
