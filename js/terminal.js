...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function processCommand(cmd) {
    ...
    switch (base) {
      ...
      case 'whoami':
        window.logToTerminal(`<span class="text-indigo-400 font-bold">Pocharam Gayathri</span> — AI/ML & data-science Engineer | Hyderabad, India`, "info");
        break;
  ...
  async function cmdActivity() {
    window.logToTerminal(`Fetching live GitHub events for <span class="text-sky-400">Gayathripocharam</span>...`, "info");
    try {
      let events = [];
      try {
        const res = await fetch('https://api.github.com/users/Gayathripocharam/events?per_page=10');
        if (!res.ok) throw new Error(`API error ${res.status}`);
        events = await res.json();
      } catch (e) {
        window.logToTerminal(`API error, falling back to local data...`, "warning");
        const fallbackRes = await fetch('data/fallback_events.json');
      ...
      events.slice(0, 8).forEach(ev => {
        ...
        const repo = ev.repo.name.replace('Gayathripocharam/', '');
