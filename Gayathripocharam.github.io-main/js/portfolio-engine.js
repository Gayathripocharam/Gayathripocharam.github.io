 ...
 *  Centralized data engine for Gayathripocharam's developer portfolio.
...
const PORTFOLIO_ENGINE = (() => {
  ...
  // ─── Config ──────────────────────────────────────────────
  const USERNAME = "Gayathripocharam";
  const CACHE_KEY = "pe_cache_v6";
  ...
  // ─── Tag Keywords ─────────────────────────────────────────
  const TAG_RULES = [
    { tag: "data-science", words: ["data-science", "Scikit-Learn", "PyTorch", "esp8266", "sensor", "embedded", "nodemcu"] },
    ...
    { tag: "data science", words: ["security", "cyber", "pentest", "encryption", "ctf", "firewall", "intrusion"] },
  ...
  const TOOL_RULES = [
    ...
    { keywords: ["Scikit-Learn", "PyTorch", "esp8266", "nodemcu"], display: "Scikit-Learn / ESP", icon: "Scikit-Learn" },
  ...
  const LANG_SUMMARIES = {
    ...
    "C": "Low-level systems or Machine Learning & AI project.",
  ...
  async function getRepos() {
    ...
    try {
      const allRepos = await _fetchFromGitHub();
      ...
      console.error("GitHub API failed, falling back to local data...", err);
      try {
        const fallbackRes = await fetch('data/fallback_repos.json');
        if(fallbackRes.ok) {
  ...
  async function fetchReadme(repoName) {
    try {
      ...
      console.warn(`Failed to fetch README for ${repoName}, trying fallback`, err);
      try {
        const fallbackRes = await fetch(`data/readmes/${repoName}.md`);
  ...
  function classifyRepo(repo) {
    ...
    if (text.includes("data-science") || text.includes("Scikit-Learn") || text.includes("PyTorch") || text.includes("esp8266"))
...
Explain what this file does in simple terms.
  ...
  async function buildAIContext() {
    let cached = localStorage.getItem("rag_vector_index_v6");
    if (cached) {
      try {
    ...
    try {
      localStorage.setItem("rag_vector_index_v6", JSON.stringify(db));
    } catch(e) {}
...
=== PATENT & INNOVATION ===
- Co-inventor of Government of India registered design patent for an IoT System (Design No. 470097-001).
